...
//        console.log('sdfdsf');
    ...
    window.app.pullDataById(document.getEmpDashboardUrl, {
        ...
        if (empData['PERFORMANCE_PERCENTAGE'] != null) {
            let performance = parseInt(empData['PERFORMANCE_PERCENTAGE']); // Convert to integer
            $('#employeePerformanceDays').text(performance + "%"); // Update text
        
            // Select the parent div and change background color based on the value
            let performanceBox = $('#employeePerformanceDays').closest('.employee_details-box');
        
            if (performance > 95) {
                performanceBox.css({ 'background-color': 'green', 'color': 'white' });
            } else if (performance > 80) {
                performanceBox.css({ 'background-color': 'blue', 'color': 'white' });
            } else if (performance > 40) {
                performanceBox.css({ 'background-color': 'green', 'color': 'black' });
            } else {
                performanceBox.css({ 'background-color': 'red', 'color': 'black' });
            }
        }
