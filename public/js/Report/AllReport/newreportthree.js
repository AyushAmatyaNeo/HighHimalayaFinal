...
(function ($, app) {
    ...
    $(document).ready(function () {
        ...
        var monthChange = function ($this) {
            ...
            // Get current date in d-MMM-y format (e.g., 01-Feb-2025)
            var currentDate = new Date();
            var day = currentDate.getDate();  // Get day (1-31)
            var month = currentDate.getMonth();  // Get month (0-11)
            var year = currentDate.getFullYear();  // Get full year (4 digits)
            
            // Format day and month with leading zeros
            var daysFormatted = day < 10 ? '0' + day : day;  // Add leading zero if day is less than 10
            var monthsFormatted = currentDate.toLocaleString('default', { month: 'short' });  // Get 3-letter month abbreviation
            
            var formattedDate = daysFormatted + '-' + monthsFormatted + '-' + year;  // Format date as 01-Feb-2025
            $toDate.val(formattedDate);  // Set the current date in the $toDate field
        
            // Convert the English date to Nepali date and set it
            var nepaliDate = nepaliDatePickerExt.fromEnglishToNepali(formattedDate);
            $nepaliFromDate.val(nepaliDate);
            $nepaliToDate.val(nepaliDate);  // You can set the Nepali date for $nepaliToDate as well
        ...
        function generateColsForKendo(daycount, monthData, leaveDetails) {
            exportVals = {
                ...
                'PERFORMANCE_PERCENT':'Punctuality Performance',
                'Overtime': {
                    'OT_DAYS': 'Days',
                    'TOTAL_OT_HOURS': 'Hour'
                },
            ...
            if (!selectedCheckboxValues || selectedCheckboxValues.trim() === "") {
                ...
                cols.push({
                    ...
                    title: "Punctuality Performance",
                    headerTemplate: "<span style='color: green; font-weight: bold;'>Punctuality Performance</span>",
                    template: function(dataItem) {
                        let color = dataItem.PERFORMANCE_PERCENT >= 95 ? "#008000" : // Green for Excellent
                    dataItem.PERFORMANCE_PERCENT >= 85 ? "#0000FF" : // Blue for Good
                    dataItem.PERFORMANCE_PERCENT >= 75 ? "#FFA500" : // Orange for Needs Improvement
                    "#FF0000"; // Red for Poor
                        return `<span style="background-color: ${color}; color: white; font-weight: bold; padding: 5px; display: block; text-align: center;">
                            ${dataItem.PERFORMANCE_PERCENT}%
                        </span>`;
                    },
                    width: 120,
                    headerAttributes: { style: "color: green; font-weight: bold;" }
                });
