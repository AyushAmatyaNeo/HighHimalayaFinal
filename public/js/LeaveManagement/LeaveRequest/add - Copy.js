    ...
    $(document).ready(function () {
        ...
        var calculateAvailableDays = function (startDateStr, endDateStr, halfDay, employeeId, leaveId) {
            ...
            app.serverRequest(document.wsFetchAvailableDays, {
                ...
                availableDays = parseFloat($availableDays.val());

                $noOfDays.val(dateDiff);
                balanceDiff = dateDiff / (halfDay === 'N' ? 1 : 2);
        ...
        var checkForErrors = function (startDateStr, endDateStr, employeeId) {
            app.pullDataById(document.wsValidateLeaveRequest, { startDate: startDateStr, endDate: endDateStr, employeeId: employeeId }).then(function (response) {
                ...
                if (response.data['ERROR'] === null && response.travelError['ERROR'] === null && (balanceDiff <= availableDays)) {
            ...
            app.pullDataById(document.wsPullLeaveDetail, {
                ...
                if (success.data.IS_SUBSTITUTE == 'Y') {
                    if (success.data.IS_SUB_LEAVE == 'Y') {
                        //  for gws onlty start
                        $('#form-remarks').prop('required', true);
                        $('#form-remarks').parent().children(':first-child').html('Remarks( mention total hours and reason) <span class="required" aria-required="true">*</span>');
                        // for gws only end
                    ...
                    //  for gws onlty start
                    $('#form-remarks').prop('required', false);
                    $('#form-remarks').parent().children(':first-child').html('Remarks');
                    // for gws only end
                    $('#request').attr("disabled", false);
        ...
        $halfDay.on('change', function () {
            ...
            if ($startDate.val() !== '' && $endDate.val() !== '') {
                ...
                checkForErrors($startDate.val(), $endDate.val(), $employee.val());
            }
        });
        ...
        var validateSubstitueLeave = function (startDate, endDate, $subDetail) {
            ...
            let subEndD = new Date($subDetail['SUB_END_DATE'].replace(/-/g, '/'));
            let subValD = new Date($subDetail['SUB_VALIDATE_DAYS'].replace(/-/g, '/'));
