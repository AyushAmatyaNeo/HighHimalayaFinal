...
class LeaveRequest extends HrisController
{
    ...
    public function viewAction()
    {
        ...
        $preBalance = $result['BALANCE_HOUR'];
    ...
    public function pullLeaveDetailAction()
    {
        try {
            ...
            if ($request->isPost()) {
                ...
                // $leaveDetail['BALANCE_HR'] = $leaveRequestRepository->checkSubstitueBalance($employeeId, $leaveId, $endDate, $leaveDetail['BALANCE_HR']);
                ...
                return new CustomViewModel(['success' => true, 'data' => $leaveDetail, 'subtituteDetails' => null, 'error' => '']);
            } else {
                throw new Exception("The request should be of type post");
            }
        } catch (Exception $e) {
    ...
    public function fetchAvailableHoursAction()
    {
        try {
            $request = $this->getRequest();
            if ($request->isPost()) {
                $postedData = $request->getPost();
                $leaveRequestRepository = new LeaveRequestRepository($this->adapter);
                $availableDays = $leaveRequestRepository->fetchAvailableHours($postedData['startDate'], $postedData['endDate'], $postedData['employeeId'], $postedData['halfDay'], $postedData['leaveId']);
    ...
    public function selfPullLeaveDetailWidEmployeeIdAction()
    {
        try {
            $request = $this->getRequest();
            if ($request->isPost()) {
    ...
    public function selfPullLeaveDetailAction()
    {
        try {
            $request = $this->getRequest();
            if ($request->isPost()) {
    ...
    public function selfFetchAvailableHoursAction()
    {
        try {
            $request = $this->getRequest();
            if ($request->isPost()) {
                $postedData = $request->getPost();
                $leaveRequestRepository = new LeaveRequestRepository($this->adapter);
                $availableDays = $leaveRequestRepository->fetchAvailableHours($postedData['startDate'], $postedData['endDate'], $this->employeeId, $postedData['halfDay'], $postedData['leaveId']);
                return new CustomViewModel(['success' => true, 'data' => $availableDays, 'error' => '']);
            } else {
                throw new Exception("The request should be of type post");
            }
    ...
    public function selfValidateLeaveRequestAction()
    {
        try {
            $request = $this->getRequest();
            if ($request->isPost()) {
                $postedData = $request->getPost();
                $leaveRequestRepository = new LeaveRequestRepository($this->adapter);
                $error = $leaveRequestRepository->validateLeaveRequest($postedData['startDate'], $postedData['endDate'], $this->employeeId);
                $travelError = $this->repository->validateLeaveTravelRequest(Helper::getExpressionDate($postedData['startDate'])->getExpression(), Helper::getExpressionDate($postedData['endDate'])->getExpression(), $this->employeeId);
                // echo '<pre>';print_r($travelError);die;
                return new CustomViewModel([
                    'success' => true, 'data' => $error, 'error' => '',
                    'travelError' => $travelError
