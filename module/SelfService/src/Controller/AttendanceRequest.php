...
class AttendanceRequest extends HrisController {
    ...
    public function addAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($this->form->isValid()) {
                ...
                $existingRequest = $this->repository->fetchByEmployeeAndDate($model->employeeId, $model->attendanceDt, $model->inTime, $model->outTime);
                //echo'<pre>';print_r( $existingRequest);die;
                if (!empty($existingRequest) && isset($existingRequest['STATUS']) && $existingRequest['STATUS'] === 'RQ') {
                    $this->flashmessenger()->addMessage('<div class="alert alert-danger" style="color: red; font-weight: bold;">
                    <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> You cannot request attendance again!
                </div>');
    ...
    public function checkInAction()
    {
        ...
        if ($id !== 0) {
            ...
            $model = new AttendanceRequestModel();
            $model->attendanceDt = $attendanceData['ATTENDANCE_DT'];
            $model->outTime = $attendanceData['IN_TIME'];
            $this->form->bind($model);
        }
        ...
        if ($request->isPost()) {
            ...
            $attendanceDetailRepo = new AttendanceRequestRepository($this->adapter);
    
            $existingAttendance = $attendanceDetailRepo->fetchByEmployeeAndDateCheckIn($this->employeeId, $model->attendanceDt, $model->inTime);
            // echo'<pre>';print_r($existingAttendance);die;
                if ($existingAttendance) {
                    $this->flashmessenger()->addMessage('<div class="alert alert-danger" style="color: red; font-weight: bold;">
                        <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> You cannot Check-In twice on the same date!
                    </div>');
            //echo'<pre>';print_r($existingAttendance);die;
                    return $this->redirect()->toRoute("attendancerequest");
                }
    ...
    public function checkOutAction()
    {
        ...
        if ($request->isPost()) {
            ...
            $existingAttendance = $attendanceDetailRepo->fetchByEmployeeAndDateCheckOut($this->employeeId, $model->attendanceDt, $model->outTime);
            // echo'<pre>';print_r($existingAttendance);die;
                if ($existingAttendance) {
                    $this->flashmessenger()->addMessage('<div class="alert alert-danger" style="color: red; font-weight: bold;">
                        <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> You cannot Check-Out twice on the same date!
                    </div>');
            //echo'<pre>';print_r($existingAttendance);die;
                    return $this->redirect()->toRoute("attendancerequest");
                }
