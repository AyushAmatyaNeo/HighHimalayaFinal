...
class AttendanceRequestRepository implements RepositoryInterface {
    ...
    public function fetchAll() {
        ...
        $select->columns([
            ...
            new Expression("INITCAP(TO_CHAR(A.ATTENDANCE_END_DT, 'DD-MON-YYYY')) AS ATTENDANCE_END_DT"),
    ...
    public function getFilterRecords($data) {
        ...
        $select->columns([
            ...
            new Expression("INITCAP(TO_CHAR(A.ATTENDANCE_END_DT, 'DD-MON-YYYY')) AS ATTENDANCE_END_DT_AD"),
            new Expression("BS_DATE(TO_CHAR(A.ATTENDANCE_END_DT, 'DD-MON-YYYY')) AS ATTENDANCE_END_DT_BS"),
    ...
    public function fetchByEmployeeAndDate($employeeId, $attendanceDt, $inTime, $outTime)
{
    $boundedParameter = [];
    $boundedParameter['employeeId'] = $employeeId;

    if ($attendanceDt) {
        $attendanceDt = str_replace(["TO_DATE('", "', 'DD-MON-YYYY')"], '', $attendanceDt->getExpression());
    } elseif ($attendanceDt) {
        $attendanceDt = $attendanceDt->format('d-M-Y'); 
    }
    $boundedParameter['attendanceDt'] = $attendanceDt;

    $sql = "SELECT STATUS, 
    TO_TIMESTAMP(TO_CHAR(TO_DATE(:attendanceDt, 'DD-MON-YYYY'), 'DD-MON-YYYY') || ' ' || 
                TO_CHAR(IN_TIME, 'HH12:MI:SS.FF9 AM'), 'DD-MON-YYYY HH12:MI:SS.FF9 AM') AS IN_TIME, 
    TO_TIMESTAMP(TO_CHAR(TO_DATE(:attendanceDt, 'DD-MON-YYYY'), 'DD-MON-YYYY') || ' ' || 
                TO_CHAR(OUT_TIME, 'HH12:MI:SS.FF9 AM'), 'DD-MON-YYYY HH12:MI:SS.FF9 AM') AS OUT_TIME
FROM (SELECT STATUS, 
          IN_TIME, 
          OUT_TIME 
   FROM HRIS_ATTENDANCE_REQUEST 
   WHERE EMPLOYEE_ID = :employeeId
   AND ATTENDANCE_DT = TO_DATE(:attendanceDt, 'DD-MON-YYYY') 
   ORDER BY ID DESC) 
WHERE ROWNUM = 1";
    ...
    $result = $statement->execute([
        ...
        'attendanceDt' => $attendanceDt,
        'inTime' => $inTime,  
        'outTime' => $outTime, 
    ]);
    return $result->current();
}
public function fetchByEmployeeAndDateCheckIn($employeeId, $attendanceDt, $inTime)
{
    $boundedParameter = [];
    $boundedParameter['employeeId'] = $employeeId;
...
public function fetchByEmployeeAndDateCheckOut($employeeId, $attendanceDt, $outTime)
{
    $boundedParameter = [];
    $boundedParameter['employeeId'] = $employeeId;
