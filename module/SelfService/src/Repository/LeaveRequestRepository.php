...
class LeaveRequestRepository implements RepositoryInterface
{
    ...
    public function add(Model $model)
    {
        ...
        // dd("lol",$model->getArrayCopyForDB());
...
employee_id,
CASE
    WHEN ( total_hr - leave_taken_till_this_month_hr ) > max_balance_hr THEN
            CASE
                WHEN max_balance_hr < 0 THEN
                    0
                ELSE
                    max_balance_hr
                -- ACTUAL_BALANCE
    ...
    ELSE
        CASE
            WHEN ( total_hr + previous_balance_hr - leave_taken_till_this_month_hr ) < 0 THEN
                    0
            ELSE
                ( total_hr - leave_taken_till_this_month_hr + previous_balance_hr )
            -- ACTUAL_BALANCE
        END
END AS balance_hr,
fiscal_year,
...
(SELECT
        ...
        WHERE
            ...
            AS leave_taken_till_this_month_hr,
(SELECT
                nvl(MAX(la.balance_hour),0)
            FROM
...
as max_balance_hr,
nvl(LA.total_hours,0) as total_hr,
nvl(LA.previous_year_bal_hr,0) as previous_balance_hr,
...
// dd($sql);
    ...
    public function getfilterRecords($data)
    {
        ...
        $select->columns([
            ...
            new Expression("TO_CHAR((CASE WHEN (LA.HALF_DAY IS NULL OR  LA.HALF_DAY = 'N') THEN LA.TOTAL_HOURS else LA.TOTAL_HOURS/2 END)) || ' Hr' AS TOTAL_HOURS"),
...
LEFT JOIN Hris_Holiday_Master_Setup H ON (WH.HOLIDAY_ID=H.HOLIDAY_ID))"], "SLR.ID=LA.SUB_REF_ID AND SLR.EMPLOYEE_ID=LA.EMPLOYEE_ID", [], "left");
    ...
    public function fetchAvailableHours($fromDate, $toDate, $employeeId, $halfDay, $leaveId)
    {
        $boundedParameter = [];
