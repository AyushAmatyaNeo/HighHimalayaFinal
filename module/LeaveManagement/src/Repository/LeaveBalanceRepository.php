...
if($isMonthly){
    ...
    FROM (SELECT *
               ...
               case when (Previous_year_bal+total-taken)<0 then 0 else  Previous_year_bal+total-taken end as balance,
                ...
                (SELECT 
                HA.EMPLOYEE_ID,
                        HA.PREVIOUS_YEAR_BAL_HR as PREVIOUS_YEAR_BAL,
                        HA.LEAVE_ID,
                        HA.TOTAL_HOURS AS TOTAL,
                        HA.BALANCE_HOUR AS BALANCE,
                        (select max(total_HOURS) from hris_employee_leave_assign where employee_id =ha.employee_id and leave_id = HA.leave_id) - HA.TOTAL_HOURS  as leave_added,
        HA.TOTAL_HOURS as max_balance,
                        HS.ENCASH_HOURS as ENCASHED,
                        ( HA.PREVIOUS_YEAR_BAL_HR + ha.total_hours - ha.balance_hour - (case when
                        HS.ENCASH_HOURS is null then 0 else HS.ENCASH_HOURS end)) AS taken,
                        (select nvl(sum(case when half_day ='N' then total_hours else total_hours/2 end),0) from hris_Employee_leave_request where status = 'AP' and leave_id = HA.leave_id
...
and (select to_date from hris_leave_month_code where leave_year_month_no = {$searchQuery['leaveYearMonthNo']} and leave_year_id = (select fiscal_year from hris_leave_master_setup where leave_id = HA.leave_id))
                        ...
                        HS.ENCASH_HOURS is null then 0 else HS.ENCASH_HOURS end) as takenNew
                FROM 
                HRIS_EMPLOYEE_LEAVE_ASSIGN HA
                        left JOIN 
                        HRIS_EMP_SELF_LEAVE_CLOSING HS
    ...
    FROM (SELECT *
                ...
                HA.EMPLOYEE_ID,
                        ...
                        HA.BALANCE_HOUR AS BALANCE,
                        HS.ENCASH_HOURS as ENCASHED,
                        nvl(PD.no_of_hours,0) as DEDUCTED,
                        ( HA.PREVIOUS_YEAR_BAL_HR + ha.total_HOURS - ha.balance_HOUR - (case when
                        HS.ENCASH_HOURS is null then 0 else HS.ENCASH_HOURS end)) AS taken
                FROM 
                HRIS_EMPLOYEE_LEAVE_ASSIGN HA
                        ...
                        HRIS_EMP_SELF_LEAVE_CLOSING HS
                        on (HA.EMPLOYEE_ID = HS.EMPLOYEE_ID and HA.leave_id = HS.leave_id)
                        left join HRIS_EMPLOYEE_PENALTY_DAYS PD
                        ON (HA.EMPLOYEE_ID = PD.EMPLOYEE_ID and HA.leave_id = PD.leave_id)
                WHERE ha.EMPLOYEE_ID IN
                    ( SELECT E.EMPLOYEE_ID FROM HRIS_EMPLOYEES E WHERE 1=1 AND E.STATUS='E' {$searchCondition['sql']}
                    ){$monthlyCondition} {$leaveCondition}
                ) PIVOT (sum ( DEDUCTED ) AS DEDUCTED, sum ( ENCASHED ) AS ENCASHED, MAX(PREVIOUS_YEAR_BAL) AS PREVIOUS_YEAR_BAL,MAX(BALANCE) AS BALANCE,MAX(TOTAL) AS TOTAL,MAX(TAKEN) AS TAKEN FOR LEAVE_ID IN ({$leaveArrayDb}) )
                ) LA LEFT JOIN HRIS_EMPLOYEES E ON (LA.EMPLOYEE_ID=E.EMPLOYEE_ID)
                LEFT JOIN HRIS_DESIGNATIONS DES
        ON E.DESIGNATION_ID=DES.DESIGNATION_ID 
        LEFT JOIN HRIS_POSITIONS P
