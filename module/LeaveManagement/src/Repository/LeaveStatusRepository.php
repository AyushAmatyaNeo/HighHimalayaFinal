...
class LeaveStatusRepository extends HrisRepository
{
    ...
    public function getFilteredRecord($data, $recomApproveId = null)
    {
        ...
        $sql = "SELECT INITCAP(L.LEAVE_ENAME) AS LEAVE_ENAME,
                  L.LEAVE_CODE,
                  (CASE WHEN (LA.HALF_DAY IS NULL OR  LA.HALF_DAY = 'N')  THEN LA.TOTAL_HOURS else la.TOTAL_HOURS/2 end) as no_of_days,
...
AS LEAVE_ENAME,
                  L.LEAVE_CODE,
                  (CASE WHEN (LA.HALF_DAY IS NULL OR  LA.HALF_DAY = 'N')  THEN LA.TOTAL_HOURS else la.TOTAL_HOURS/2 end)||' hr' as no_of_hours,
                  case when L.ALLOW_HALFDAY = 'Y'
                  then LA.NO_OF_DAYS/2
                  else LA.NO_OF_DAYS
                  END as ACTUAL_DAYS,
