...
class SalSheEmpDetRepo extends HrisRepository
{
    ...
    LEFT JOIN HRIS_FLAT_VALUE_DETAIL FD1 ON (FD1.EMPLOYEE_ID = SSD.EMPLOYEE_ID AND FD1.FLAT_ID in (select flat_id from hris_flat_value_setup where flat_code='BS') AND FD1.FISCAL_YEAR_ID IN (SELECT FISCAL_YEAR_ID FROM HRIS_MONTH_CODE WHERE MONTH_ID = :monthId))
    LEFT JOIN HRIS_FLAT_VALUE_DETAIL FD2 ON (FD2.EMPLOYEE_ID = SSD.EMPLOYEE_ID AND FD2.FLAT_ID in (select flat_id from hris_flat_value_setup where flat_code='DA1') AND FD2.FISCAL_YEAR_ID IN (SELECT FISCAL_YEAR_ID FROM HRIS_MONTH_CODE WHERE MONTH_ID = :monthId))
