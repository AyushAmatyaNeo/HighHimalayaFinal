...
and Show_Default='Y'  AND VARIABLE_TYPE='O'";
    ...
    {
        // echo'<pre>';print_r("maintenance");die;
        ...
        $companyId = -1;
        $companyIdNew = $data['companyId'] ? $data['companyId'] : -1;

        if ($companyIdNew <> -1) {
            // Convert the array values into a comma-separated string for the SQL IN clause
            $companyIds = implode(',', $companyIdNew);

            // Create the condition for SQL WHERE IN
            $companyConditionNew = " AND SSED.company_id IN ($companyIds)";
        } else {
            $companyConditionNew = "";
        }
        // $companyId = isset($data['companyId']) ? $data['companyId'] : -1;
    ...
    public function getGroupDetailReport($data)
    {
        ...
        if ($groupVariable) {
            ...
            $groupVariableCondition = "and V.VARIANCE_ID=$groupVariable";
        }
