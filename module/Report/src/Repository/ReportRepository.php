...
ORDER BY UPPER(HG.GENDER_NAME)";
  ...
  public function renumerationReport($data)
  {
    ...
    left join hris_salary_sheet_detail hssd1 on (hssd1.employee_id=hssed.employee_id and hssd1.sheet_no=hssed.sheet_no and hssd1.pay_id in (select pay_id from hris_pay_setup where pay_code='BS'))
    left join hris_salary_sheet_detail hssd2 on (hssd2.employee_id=hssed.employee_id and hssd2.sheet_no=hssed.sheet_no and hssd2.pay_id in (select pay_id from hris_pay_setup where pay_code='DA'))
    left join hris_salary_sheet_detail hssd3 on (hssd3.employee_id=hssed.employee_id and hssd3.sheet_no=hssed.sheet_no and hssd3.pay_id in (select pay_id from hris_pay_setup where pay_code='GS'))
    left join hris_salary_sheet_detail hssd4 on (hssd4.employee_id=hssed.employee_id and hssd4.sheet_no=hssed.sheet_no and hssd4.pay_id in (select pay_id from hris_pay_setup where pay_code='SSF31'))
    left join hris_salary_sheet_detail hssd5 on (hssd5.employee_id=hssed.employee_id and hssd5.sheet_no=hssed.sheet_no and hssd5.pay_id in (select pay_id from hris_pay_setup where pay_code='RT'))
...
ROUND(
    GREATEST(
        (( 
            (CL.PRESENT - CL.LATEIN_DAYS - CL.EARLYOUT_DAYS - CL.MISSPUNCH_DAYS) * 
            (SELECT CREDIT AS PRESENT FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Present') 
            + ( CL.LEAVE * NVL((SELECT CREDIT AS LEAVE FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Leave') , 0 )) + ( CL.DAYOFF * NVL((SELECT CREDIT AS DAYOFF FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Dayoff'),0 )) + ( CL.HOLIDAY * NVL ( ( SELECT CREDIT AS HOLIDAY FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Holiday') ,0 )) + (CL.TRAVEL * NVL ( ( SELECT CREDIT AS TRAVEL FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Travel') ,0 )) + (CL.TRAINING * NVL((SELECT CREDIT AS TRAINING FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Training'),0 ))
            + (CL.LATEIN_DAYS * NVL((SELECT CREDIT AS LATEIN_DAYS FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Late In'), 0))  
            + (CL.ABSENT * NVL((SELECT CREDIT AS ABSENT FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Absent'),0)) + (CL.MISSPUNCH_DAYS * NVL((SELECT CREDIT AS MISSPUNCH_DAYS FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Miss Punch') , 0))
            + (CL.EARLYOUT_DAYS * NVL((SELECT CREDIT AS EARLYOUT_DAYS FROM HRIS_PERFORMANCE_SETUP WHERE category_name='Early Out') , 0))
        ) / NVL((CL.PRESENT+CL.ABSENT+CL.LEAVE+CL.DAYOFF+CL.HOLIDAY+CL.WORK_DAYOFF+CL.WORK_HOLIDAY+CL.TRAINING+CL.TRAVEL+CL.EVENT_CONF) , 0)) * 100, 0), 
    0
...
// echo'<pre>';print_r($sql);die;
  ...
  public function userVacancyReport(){
      $sql = "SELECT ROWNUM, ID,POSITION,FULL_NAME,EMAIL,CONTACT_NUMBER,COVER_LETTER,RESUME_PATH, VACANCY_STATUS_DESC(STATUS) as VACANCY_STATUS, APPLICANT_QUESTION, FEFRENCE_FROM from hris_vacancy_application order by id desc";
      $statement = $this->adapter->query($sql);
      $result = $statement->execute();
  ...
  public function fetchDataForView(){
    $sql = "SELECT ROWNUM,ID,POSITION,FULL_NAME,EMAIL,CONTACT_NUMBER,COVER_LETTER from hris_vacancy_application";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
...
public function pullApplicationReport($status){
  $whereCondition='';
  if($status!='AA'){
    $whereCondition .= " AND status in ('$status')";
  }
  $sql = "SELECT 
    hva.ID,
    hva.POSITION,
    ...
    hva.COVER_LETTER,
    hva.RESUME_PATH,
    VACANCY_STATUS_DESC(hva.STATUS) AS VACANCY_STATUS,
    CASE 
        WHEN hic.evaluator_rating_1 = 5 THEN 'Excellent'
        WHEN hic.evaluator_rating_1 = 4 THEN 'Very Good'
        WHEN hic.evaluator_rating_1 = 3 THEN 'Good'
        WHEN hic.evaluator_rating_1 = 2 THEN 'Medium'
        WHEN hic.evaluator_rating_1 = 1 THEN 'Poor'
        ELSE 'No Rating'
    END AS evaluator_rating_1, evaluator_name_1,
    CASE 
        WHEN hic.evaluator_rating_2 = 5 THEN 'Excellent'
        WHEN hic.evaluator_rating_2 = 4 THEN 'Very Good'
        WHEN hic.evaluator_rating_2 = 3 THEN 'Good'
        WHEN hic.evaluator_rating_2 = 2 THEN 'Medium'
        WHEN hic.evaluator_rating_2 = 1 THEN 'Poor'
        ELSE 'No Rating'
    END AS evaluator_rating_2, evaluator_name_2,
    CASE 
        WHEN hic.evaluator_rating_3 = 5 THEN 'Excellent'
        WHEN hic.evaluator_rating_3 = 4 THEN 'Very Good'
        WHEN hic.evaluator_rating_3 = 3 THEN 'Good'
        WHEN hic.evaluator_rating_3 = 2 THEN 'Medium'
        WHEN hic.evaluator_rating_3 = 1 THEN 'Poor'
        ELSE 'No Rating'
   END AS evaluator_rating_3,
hic.evaluator_name_3,
NVL(hic.evaluator_rating_1, 0) AS RATING1,
NVL(hic.evaluator_rating_2, 0) AS RATING2,
NVL(hic.evaluator_rating_3, 0) AS RATING3,
ROUND((NVL(hic.evaluator_rating_1, 0) + NVL(hic.evaluator_rating_2, 0) + NVL(hic.evaluator_rating_3, 0)) / 3) AS average



FROM 
    hris_vacancy_application hva
LEFT JOIN 
    hris_interview_call hic 
ON 
    hva.id = hic.id
WHERE 
    1 = 1 $whereCondition";
...
public function updateApplicationStatus($id,$status) {
  ...
  $sql = "update hris_vacancy_application  set status='$status' where id=$id ";
  $statement = $this->adapter->query($sql);
  $result = $statement->execute();
 // echo '<pre>';print_r($sql);die;
  return Helper::extractDbData($result);
...
public function getName($id) {
  ...
  $sql = "select full_name, contact_number from  hris_vacancy_application where id=$id ";
  $statement = $this->adapter->query($sql);
  $result = $statement->execute();
 // echo '<pre>';print_r($sql);die;
  return Helper::extractDbData($result);
...
public function insertInterviewStatus($data) 
{
    $sql = "INSERT INTO hris_interview_call (
        id,
        full_name,
        permanent_address,
        temporary_address,
        ...
        :expected_salary,
        :negotiation,
        TO_DATE(:joining_date, 'DD-MON-RR'),
        :remarks,
        :evaluator_name_1,
        :evaluator_remarks_1,
        :evaluator_rating_1,
  ...
  foreach ($data as $key => $value) {
      $placeholder = ':' . $key;
      $statement->getParameterContainer()->offsetSet($placeholder, $value);
  }
...
public function interViewAllReport(){
  $sql = "SELECT * from hris_interview_call order by ID DESC";
  $statement = $this->adapter->query($sql);
  $result = $statement->execute();
...
public function rejectApplication($id,$status) {
...
public function fetchInterViewData($id) {
 
  $sql = "select * from  hris_interview_call where id=$id ";
  $statement = $this->adapter->query($sql);
  $result = $statement->execute();
 // echo '<pre>';print_r($sql);die;
  return Helper::extractDbData($result);
...
public function updateInterviewApplication($interviewId, $data)
{
  $sql = "UPDATE hris_interview_call 
  SET full_name = '{$data['full_name']}', 
      ...
      job_shifting_reason = '{$data['job_shifting_reason']}', 
      vehicle_yn = '{$data['vehicle_yn']}', 
...
//  dd($module);
$sql = "SELECT * FROM HRIS_LOGS WHERE 1=1";

if (!empty($data['operation'])) {
    $operation = $data['operation']; // $operation is an array
    $operationList = implode("','", $operation); // Convert the array to a comma-separated string for SQL
    $sql .= " AND operation IN ('$operationList')"; // Use IN for multiple values
}
if (!empty($data['module'])) {
  $module = $data['module']; // $module is an array
  $moduleList = implode("','", $module); // Convert the array to a comma-separated string for SQL
  $sql .= " AND UPPER(module) IN ('$moduleList')"; // Use IN for multiple values
}
...
{
  $sql = "select INDEX_ID,PERCENT_RANGE,PERCENT_DESC from HRIS_PERFORMANCE_INDEX";
