...
where SS.SHEET_NO=:sheetNo";
   ...
   
   {
    ...
    public function getPayslipData($sheetNo)
    {
        ...
        hris_salary_sheet hss where (hssd.sheet_no=hssed.sheet_no and hssd.employee_id=hssed.employee_id) and  (hssd.sheet_no=hss.sheet_no)and hssd.sheet_no=$sheetNo and hssed.sms_flag='N'  order by hssd.employee_id desc";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
    ...
    public function addSendPayslip($id, $employeeId, $createdEmp, $type)
    {
        $sql = "Insert into HRIS_PAYSLIP_EMAIL (id,employee_id,created_by,created_dt,type) values ($id,$employeeId,$createdEmp,trunc(sysdate),'$type')";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        $result->current();
    }

    public function lastPayslipEmp()
    {
        $sql = "SELECT employee_id
        FROM hris_payslip_email
        WHERE id = (SELECT MAX(id) FROM hris_payslip_email)";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        return $result->current();
    }

    public function updateSalEmpDet($data)
    {
        $employeeId = $data['EMPLOYEE_ID'];
        $monthId = $data['MONTH_ID'];
        $sheetNo = $data['SHEET_NO'];
        $sql = "Update HRIS_SALARY_SHEET_EMP_DETAIL set sms_flag='Y' where employee_id=$employeeId  and month_id=$monthId and sheet_no=$sheetNo";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        $result->current();
    }
    public function updateSalEmpDetSalary($data)
    {
        $employeeId = $data['EMPLOYEE_ID'];
        $sql = "Update HRIS_SALARY_SHEET_EMP_DETAIL set salary_flag='Y' where employee_id=$employeeId";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        $result->current();
    }
