...
//print_r($sql);die;
 ...
 and sheet_no in (select sheet_no from hris_salary_sheet where month_id = ($monthId-1) and salary_type_id = 1)
 and pay_id = 106
          ";
        $resultList = $this->rawQuery($sql);
      // echo'<pre>'; print_r($sql);die;
    ...
    public function getOverTimeAddition($employeeId,$monthId)
    {
        $sql="
        select ADDITION AS OVERTIME_ADDITION from hris_overtime_a_d WHERE MONTH_ID=$monthId AND EMPLOYEE_ID=$employeeId
          ";
        $resultList = $this->rawQuery($sql);
      // echo'<pre>'; print_r($sql);die;
        if (!(sizeof($resultList) == 1)) {
      ...
      // echo'<pre>'; print_r($sql);die;
        ...
        return $resultList[0]['OVERTIME_ADDITION'];
    }
    ...
    {
        $sql="
        SELECT CASE WHEN POSITION_ID IN(12,13) THEN 1 ELSE 0 END AS POSITION_ID FROM HRIS_EMPLOYEES WHERE STATUS='E' AND EMPLOYEE_ID = $employeeId";
        $resultList = $this->rawQuery($sql);
      // echo'<pre>'; print_r($sql);die;
        if (!(sizeof($resultList) == 1)) {
            return 0;
 ...
 public function getNormalEligible($employeeId,$monthId)
    {
        $sql="select val as NORMAL_ELIGIBLE_DEDUCTION from hris_salary_sheet_detail where employee_id = $employeeId
 and sheet_no in (select sheet_no from hris_salary_sheet where month_id = ($monthId-1) and salary_type_id = 1)
 and pay_id = 109
          ";
        $resultList = $this->rawQuery($sql);
      ...
      // echo'<pre>'; print_r($sql);die;
        ...
        return $resultList[0]['NORMAL_ELIGIBLE_DEDUCTION'];
    }
