...
class EmployeeRepository extends HrisRepository implements RepositoryInterface
{
    ...
    public function __construct(AdapterInterface $adapter, $tableName = null)
    {
        ...
        $this->vdcGateway = new TableGateway('HRIS_MUNICIPALITIES', $adapter);
    ...
    public function getById($id)
    {
        ...
        $select
            ...
            ->join(['VM' => "HRIS_MUNICIPALITIES"], "E." . HrEmployees::ADDR_PERM_VDC_MUNICIPALITY_ID . "=VM.VDC_MUNICIPALITY_ID", ['VDC_MUNICIPALITY_NAME' => new Expression('INITCAP(VM.VDC_MUNICIPALITY_NAME)')], 'left')
            ...
            ->join(['VM1' => "HRIS_MUNICIPALITIES"], "E." . HrEmployees::ADDR_TEMP_VDC_MUNICIPALITY_ID . "=VM1.VDC_MUNICIPALITY_ID", ['VDC_MUNICIPALITY_NAME_TEMP' => 'VDC_MUNICIPALITY_NAME'], 'left')
    ...
    public function filterRecords($employeeId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $getResult = null, $companyId = null, $employeeTypeId = null)
    {
        ...
        $sql = "SELECT E.EMPLOYEE_ID                                                AS EMPLOYEE_ID,
            ...
            LEFT JOIN HRIS_MUNICIPALITIES VM
            ON E.ADDR_PERM_VDC_MUNICIPALITY_ID=VM.VDC_MUNICIPALITY_ID
            LEFT JOIN HRIS_MUNICIPALITIES VM1
    ...
    public function fetchBy($by)
    {
        ...
        $sql = "SELECT 
            {$columIfSynergy}
                ...
                LEFT JOIN HRIS_MUNICIPALITIES VMP
                ...
                LEFT JOIN HRIS_MUNICIPALITIES VMT
    ...
    public function vdcStringToId($districtId, $vdc)
    {
        ...
        if ($current != null) {
            ...
            $id = ((int) Helper::getMaxId($this->adapter, "HRIS_MUNICIPALITIES", "VDC_MUNICIPALITY_ID")) + 1;
            $insertSql = "INSERT INTO HRIS_MUNICIPALITIES (VDC_MUNICIPALITY_ID,VDC_MUNICIPALITY_NAME,DISTRICT_ID,STATUS) VALUES({$id},'{$vdc}',{$districtId},'E')";
    ...
    public function employeeDetailSession($id)
    {
        ...
        $sql = "
                SELECT E.EMPLOYEE_ID                                                AS EMPLOYEE_ID,
                  ...
                  E.PROFILE_PICTURE_ID                                              AS PROFILE_PICTURE_ID,
                  E.RETIRED_FLAG                                                    AS RETIRED_FLAG,
                  E.APPRAISAL_FLAG                                                    AS APPRAISAL_FLAG,
                  E.EMPLOYEE_TYPE                                                   AS EMPLOYEE_TYPE,
                  E.CREATED_BY                                                      AS CREATED_BY,
                  E.MODIFIED_BY                                                     AS MODIFIED_BY,
                  E.MODIFIED_DT                                                     AS MODIFIED_DT,
...
GROUP BY IARA.EMPLOYEE_ID) AA ON (AA.EMPLOYEE_ID=E.EMPLOYEE_ID)
  ...
  FROM (SELECT EXCEPTIONAL  FROM  HRIS_DEPARTMENTS WHERE DEPARTMENT_ID IN  (INVALUES))
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL
    ...
    public function getLetterByEmpId($employeeId) {
        $boundedParams = [];
        $boundedParams['employee_id'] = $employeeId;
    
        if ($employeeId == null) {
            return [];
        }
        $sql = "SELECT HEL.EMPLOYEE_ID, HEL.LETTER_SETUP_ID, HLS.LETTER_TITLE as TITTLE
                FROM HRIS_EMPLOYEE_LETTER_ASSIGN HEL
                LEFT JOIN HRIS_LETTER_SETUP HLS
                ON HEL.LETTER_SETUP_ID = HLS.LETTER_SETUP_ID
