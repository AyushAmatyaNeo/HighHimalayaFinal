   ...
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL
    ...
    public function fetchById($id)
    {
        ...
        $select->columns([
            ...
            new Expression("INITCAP(TO_CHAR(H.TO_CONTRACT_EXPIRY_DATE, 'DD-MON-YYYY')) AS TO_CONTRACT_EXPIRY_DATE"),
        ], true);
        $select->from(['H' => "HRIS_JOB_HISTORY"]);
    ...
    function fetchByEmployeeId($employeeId)
    {
        ...
        $result = EntityHelper::rawQueryResult($this->adapter, "
            SELECT INITCAP(TO_CHAR(H.START_DATE, 'YYYY-MM-DD')) AS START_DATE,
              ...
              INITCAP(TO_CHAR(H.EVENT_DATE, 'YYYY-MM-DD'))        AS EVENT_DATE,
              H.EMPLOYEE_ID                                      AS EMPLOYEE_ID,
              H.JOB_HISTORY_ID                                   AS JOB_HISTORY_ID,
              H.SERVICE_EVENT_TYPE_ID                            AS SERVICE_EVENT_TYPE_ID,
              ...
              H.TO_SALARY                                        AS TO_SALARY,
              H.TO_CONTRACT_EXPIRY_DATE                          AS TO_CONTRACT_EXPIRY_DATE
            FROM HRIS_JOB_HISTORY H 
            JOIN HRIS_SERVICE_EVENT_TYPES SE ON (H.SERVICE_EVENT_TYPE_ID = SE.SERVICE_EVENT_TYPE_ID)
            LEFT JOIN HRIS_COMPANY C ON (H.TO_COMPANY_ID = C.COMPANY_ID)
            LEFT JOIN HRIS_BRANCHES B ON (H.TO_BRANCH_ID = B.BRANCH_ID)
            LEFT JOIN HRIS_DEPARTMENTS DEP ON (H.TO_DEPARTMENT_ID = DEP.DEPARTMENT_ID)
            LEFT JOIN HRIS_DESIGNATIONS DES ON (H.TO_DESIGNATION_ID = DES.DESIGNATION_ID)
            LEFT JOIN HRIS_POSITIONS P ON (H.TO_POSITION_ID = P.POSITION_ID)
            JOIN HRIS_SERVICE_TYPES ST ON (H.TO_SERVICE_TYPE_ID = ST.SERVICE_TYPE_ID)
            WHERE H.EMPLOYEE_ID = :employeeId AND H.STATUS = 'E'
