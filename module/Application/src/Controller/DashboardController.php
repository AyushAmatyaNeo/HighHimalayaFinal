...
class DashboardController extends AbstractActionController {
    ...
    public function changeCalendarAction(){
        $calendarType = $_POST['calenderLg'];
        $dashboardRepo = new DashboardRepository($this->adapter);
        $employee_id=$this->employeeId;
        $dashboardRepo->changeCalendarType($employee_id,$calendarType);
    ...
    public function indexAction() {
        ...
        $dashboardRepo = new DashboardRepository($this->adapter);
        $employee_id=$this->employeeId;
        $calendarLg=$dashboardRepo->getCalendarType($employee_id);
        $companyId=$this->auth->getStorage()->read()['employee_detail']['COMPANY_ID'];

        $calenderType=$calendarLg[0]['CALENDAR_TYPE'];
        if(isset($this->auth->getStorage()->read()['preference']['calendarView'])){
            if($calenderType == null){
                $calenderRef=$this->auth->getStorage()->read()['preference']['calendarView'];
                $dashboardRepo->createReference($employee_id,$calenderRef);
            }
        }
        $calenderType=$calendarLg[0]['CALENDAR_TYPE'];
        $performancePercentage=$dashboardRepo->getPerformancePercentage();
        //echo '<pre>';print_r($performancePercentage);die;
        $data = [
    ...
    public function branchManagerAction()
    {
        ...
        $data = [
            "employeeNotice" => $dashboardRepo->fetchEmployeeNotice($this->employeeId),
            "employeeTask" => $dashboardRepo->fetchEmployeeTask($this->employeeId),
            "employeesBirthday" => $dashboardRepo->getManagerDashboardEmployeeBirthdays($this->employeeId),
            "employeeList" => $dashboardRepo->getManagerDashboardAllEmployee($this->employeeId),
            "headCountGender" => $dashboardRepo->getManagerDashboardGenderHeadCount($this->employeeId),
            "headCountDepartment" => $dashboardRepo->getManagerDashboardDepartmentHeadCount($this->employeeId),
            "headCountLocation" => $dashboardRepo->fetchLocationHeadCount(),
            "departmentAttendance" => $dashboardRepo->getManagerDashboardDepartmentAttendance($this->employeeId),
            'todoList' => $this->getTodoList(),
            "upcomingHolidays" => $dashboardRepo->fetchUpcomingHolidays($this->employeeId),
            "newEmployees" => $dashboardRepo->getManagerDashboardJoinedEmployees($this->employeeId),
            "leftEmployees" => $dashboardRepo->fetchLeftEmployees(),
        ];
        $view = new ViewModel(Helper::addFlashMessagesToArray($this, $data));
        $view->setTemplate("dashboard/branch-manager");
