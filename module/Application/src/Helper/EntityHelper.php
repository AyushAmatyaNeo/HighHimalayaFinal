...
class EntityHelper
{
    ...
    public static function getSearchConditonBoundedJGI($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, $genderId = null, $locationId = null, $functionalTypeId = null)
    {
        $conditon = "";
        $allParameters = [];
      ...
      FROM (SELECT EXCEPTIONAL  FROM  HRIS_DEPARTMENTS WHERE DEPARTMENT_ID IN  (INVALUES))
       CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL
        ...
        if ($serviceTypeId != null && $serviceTypeId != -1) {
            ...
            $conditon .= " AND (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE  IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL)";
        }
   ...
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL)");
    ...
    public static function getRAWiseEmployeeList($adapter, $eid, $returnType = 0)
    {
        $sql = "SELECT * FROM (select e.employee_id as employee_id, 
                ...
                join hris_employees e on (e.employee_id = ra.employee_id) where  e.status='E' AND (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE NOT IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL)  and Approved_By = $eid
                or Recommend_By = $eid)
                union
                ...
                join hris_employees e on (e.employee_id = ra.employee_id) where e.status='E' AND (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE NOT IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL) and r_a_id = $eid)";
        $result = self::rawQueryResult($adapter, $sql);
