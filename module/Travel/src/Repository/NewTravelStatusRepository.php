...
class NewTravelStatusRepository extends HrisRepository
{
    ...
    public function getFilteredRecord($search): array
    {
        ...
        $sql = "SELECT TR.TRAVEL_ID                        AS TRAVEL_ID,
                ...
                WHERE 1          =1 AND TR.TRAVEL_TYPE IS NOT NULL {$condition} order by TR.FROM_DATE desc
                -- AND TR.from_date BETWEEN '{$startdate}' AND '{$enddate}'
                -- AND TR.to_date BETWEEN '{$startdate}' AND '{$enddate}'
                ";
    ...
    public function notSettled(): array
    {
        $sql = "SELECT TR.TRAVEL_ID                   AS TRAVEL_ID,
                ...
                ON (RA.RECOMMEND_BY=RAR.EMPLOYEE_ID)
                LEFT JOIN HRIS_EMPLOYEES RAA
                ON(RA.APPROVED_BY=RAA.EMPLOYEE_ID) ORDER BY TR.FROM_DATE DESC";
  ...
  AND EMPLOYEE_ID = $employeeId
    ...
    {
        $sql = "DELETE FROM HRIS_TRAVEL_FILES WHERE FILE_ID=$id";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        return $result->current();
    }
