    ...
    */
    ...
    public function bulkAttendanceWSAction()
    {
        try {
            ...
            $outTimeLogs = $postedData['out_time'];
            $inTimeLogs =  $postedData['in_time'];
            $createdBy = $this->employeeId;

           // echo'<pre>';print_r($createdBy);die;
            //return new JsonModel(['success' => true, 'data' => $postedData, 'error' => '']);
            $this->repository->manualAttendance($postedData['employeeId'], Helper::getExpressionDate($postedData['attendanceDt'])->getExpression(), $postedData['action'], $postedData['impactOtherDays'] === 'true', $postedData['shiftId'], $inTime, $outTime, $outTimeLogs,$inTimeLogs, $postedData['outNextDay'] === 'true', $createdBy);
