...
class AttendanceDetailRepository implements RepositoryInterface
{
    ...
    public function fetchEmployeeShfitDetails($employeeId)
    {
        ...
        $statement = $this->adapter->createStatement($sql, [':employeeId' => $employeeId]);
        $result = $statement->execute();
        return $result->current();
    }
    ...
    public function manualAttendance($employeeId, $attendanceDt, $action, $impactOtherDays, $shiftId = null, $in_time = null, $out_time = null, $out_time_logs = null, $in_time_logs = null, $outNextDay = false, $createdBy)
    {
        if ($impactOtherDays) {
            $sql = "BEGIN
        ...
        // $logs = new Logs();
        // $logs->module = 'Manual Attendance from HR';
        // $logs->operation = 'U';
        // $logs->createdBy = $createdBy;
        // $logs->createdDesc = "Modified By - {$createdBy} | In Time - {$in_time_logs} | Out Time - {$out_time_logs}";
