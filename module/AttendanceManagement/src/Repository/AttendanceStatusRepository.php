...
//                     THEN ('E')
//                   END
//                 OR APRV.STATUS IS NULL) {$searchCondition['sql']} {$statusCondition} {$fromDateCondition} {$toDateCondition} order by AR.ATTENDANCE_DT desc";
...
class AttendanceStatusRepository extends HrisRepository
{
  ...
  public function getFilteredRecord($data, $recomApproveId)
  {
    ...
    if ($fromDate != null) {
      $fromDateCondition = " AND AR.ATTENDANCE_DT >= TO_DATE('{$fromDate}', 'DD-MM-YYYY') AND AR.ATTENDANCE_END_DT >= TO_DATE('{$fromDate}', 'DD-MM-YYYY')";
    }

    if ($toDate != null) {
      $toDateCondition = " AND AR.ATTENDANCE_DT <= TO_DATE('{$toDate}', 'DD-MM-YYYY') AND AR.ATTENDANCE_END_DT <= TO_DATE('{$toDate}', 'DD-MM-YYYY')";
    }
    ...
    $sql = "SELECT DISTINCT AR.ID                                             AS ID,
                  ...
                  INITCAP(TO_CHAR(AR.ATTENDANCE_DT, 'DD-MON-YYYY'))               AS ATTENDANCE_DT_AD,
                  BS_DATE(TO_CHAR(AR.ATTENDANCE_DT, 'DD-MON-YYYY'))               AS ATTENDANCE_DT_BS,
                  INITCAP(TO_CHAR(AR.ATTENDANCE_END_DT, 'DD-MON-YYYY'))          AS ATTENDANCE_END_DT_AD,
                  BS_DATE(TO_CHAR(AR.ATTENDANCE_END_DT, 'DD-MON-YYYY'))           AS ATTENDANCE_END_DT_BS,
                  INITCAP(TO_CHAR(AR.IN_TIME, 'HH:MI AM'))                        AS IN_TIME,
                  INITCAP(TO_CHAR(AR.OUT_TIME, 'HH:MI AM'))                       AS OUT_TIME,
                  AR.IN_REMARKS                                                   AS IN_REMARKS,
                  AR.OUT_REMARKS                                                  AS OUT_REMARKS,
  ...
  public function getAttenReqList($data): array
  {
    ...
    if ($fromDate != null) {
      $fromDateCondition = " AND AR.ATTENDANCE_DT >= :fromDate AND AR.ATTENDANCE_END_DT >= :fromDate";
      $boundedParams['fromDate'] = $fromDate;
    }

    if ($toDate != null) {
      $toDateCondition = " AND AR.ATTENDANCE_DT <= :toDate AND AR.ATTENDANCE_END_DT <= :toDate";
      $boundedParams['toDate'] = $toDate;
    }
